%% Neural Style Transfer Using Deep Learning Adapted for Gifs
...
gifImg = imresize(gifImage,imageSize);
%% 
...
gifImg = rescale(single(gifImg),0,255) - meanVggNet;
...
for frame = 1 : size(gifImage, 4)
    ...
    for iteration = 1:numIterations
        ...
        if mod(iteration,50) == 0 || (iteration == 1)
            ...
            transferImage(:,:,:,frame) = gather(extractdata(dlTransfer));
            transferImage(:,:,:,frame) = transferImage(:,:,:,frame) + meanVggNet;
            % transferImage(:,:,:,frame) = uint8(transferImage(:,:,:,frame));
            %transferImage(:,:,:,frame) = imresize(transferImage(:,:,:,frame),size(gifImage,[1 2]));
            
            image(transferImage(:,:,:,frame) ./ 255)
    ...
    transferImage(:,:,:,frame) = gather(extractdata(dlOutput));
    %
...
gifwrite(transferImage, 'practice-transfer.gif')
